...
local function saveCallback(w)
			...
			if (switchType ~= "Projectiles") then
				for key, value in pairs(unitTable) do
					if (v.ID == value.ID) then
						...
						value.UpgradeCost = v.UpgradeCost:value()
						value.UpgradeTime = v.UpgradeTime:value()
	...
	for i = 0, 153 do
		...
		reading = string.sub(reading, 1, base + 98) .. ThisLittleHexIsPayback(a.UpgradeCost) .. string.sub(reading, base + 101, string.len(reading))
		reading = string.sub(reading, 1, base + 100) .. ThisLittleHexIsPayback(a.UpgradeTime) .. string.sub(reading, base + 103, string.len(reading))
...
local function switchCallback(w)
	...
	for k, v in pairs(tempPeople) do
		...
		tpos = tpos + 155
		b.UpgradeCost = fltk:Fl_Value_Input(tpos, yPosition, 50, 25, "Upgrade Cost")
		b.UpgradeCost:labelsize(14)
		b.UpgradeCost:textsize(14)
		b.UpgradeCost:minimum(0)
		b.UpgradeCost:maximum(65535)
		b.UpgradeCost:step(5)
		b.UpgradeCost:value(a.UpgradeCost)
		
		tpos = tpos + 155
		b.UpgradeTime = fltk:Fl_Value_Input(tpos, yPosition, 50, 25, "Upgrade Time")
		b.UpgradeTime:labelsize(14)
		b.UpgradeTime:textsize(14)
		b.UpgradeTime:minimum(0)
		b.UpgradeTime:maximum(65535)
		b.UpgradeTime:step(5)
		b.UpgradeTime:value(a.UpgradeTime)
...
for i = 0, 153 do 
	...
	a.UpgradeCost = NothingICantHandle(base + 99, base + 100)
	a.UpgradeTime = NothingICantHandle(base + 101, base + 102)
